...
for m in range(100):
    ...
    result =[]
    for n in range(900,1400,10): #100,1000,10 for all + 900,1400,10 for gower
        random.seed(m)
    ...
    df.to_csv(output_path+percolation_folder+'/resolution1000_all_'+method+'1.csv')

best_df = df.sort_values(by='num_10members_communities', ascending=False).groupby(['method','random_state']).head(1)
best_df.to_csv(output_path+percolation_folder+'/resolution1000_count_'+method+'1.csv')
best_count = best_df.groupby('method')['resolution'].value_counts().sort_values(ascending=False).head(5)
best_count.to_csv(output_path+percolation_folder+'/resolution1000_'+method+'1.csv')
print(best_count)
