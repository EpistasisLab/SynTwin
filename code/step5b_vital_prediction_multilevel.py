...
def outcome_prediction_multilevel(filepath, threshold=0.5, resolution=100, random_state=46):
    ...
    random.seed(random_state)
    communities = g.community_multilevel(weights=g.es['weight'], resolution=resolution)
    ...
    for subgraph in communities.subgraphs():
        if subgraph.vcount()>=10:
            ...
            community_members = list(subgraph.vs['name']) #[literal_eval(mem) for mem in subgraph.vs['name']]
    ...
    print(f"saved all_predict to {output_path}")
    ...
    print("step5 outcome prediction with multilevel community completed")
